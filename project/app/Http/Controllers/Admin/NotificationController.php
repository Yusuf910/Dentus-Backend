...
class NotificationController extends Controller
{
    ...
    {
        $page_title = 'Notification User';
        return view('doctor.notification.user', compact('page_title'));
    }
    ...
    public function send_notification_to_user(Request $request)
    {
        ...
        if ($request->ajax()) {
            $name = $request->name;
            $template['search'] = UserProfile::where('link_id','=', auth()->user()->id)->where(function ($query) use ($name) {
            ...
            if (count($template['search']) > 0) {
                ...
                $output = '<table class="table table-striped" style="display:block;position:relative;z-index:1">';
        ...
        return view('doctor.notification.user', $template);
    }
 ...
 public function smt_notification(Request $request)
{
    ...
    if ($request->hasFile("image")) {
        ...
        $imageName = time().'.'.$sfile->extension();
        $sfile->move(public_path('notification/'), $imageName);
        $image = $imageName;
    }

    if ($image) {
        $image_url = asset('notification/'.$image);
    } else {
        $image_url = '';
    }
    ...
    if ($request->filled('user_type')) {
        ...
        $users = UserProfile::where('status',1)
                ->whereNotNull('device_token')
                ->pluck('device_token');

        foreach ($users as $token) {
            $this->send($token,$title,$body,$image_url);
        }
    ...
    if ($request->filled('id')) {
        ...
        if ($user && $user->device_token) {
            $this->send($user->device_token,$title,$body,$image_url);
        }

        return redirect()->back()->with('success','Notification sent to user');
    }
    ...
    public function send_notification_to_astrologer(Request $request)
    {
        if ($request->ajax()) {
            $name = $request->name;
            $template['search'] = User::where('status', 1)->where(function ($query) use ($name) {
                $query->where('name', 'LIKE', '%' . $name . '%');
                $query->orWhere('mobile', 'LIKE', '%' . $name . '%');
            })
                ->get();
    ...
    {
        return view('doctor.notification.all');
    }
}
