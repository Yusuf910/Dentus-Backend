...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $currentAppointments = Booking::where('doctor_id', $doctorId)
            ->whereBetween('created_at', [$startOfCurrentMonth, $endOfCurrentMonth])
            ->count();

        $lastMonthAppointments = Booking::where('doctor_id', $doctorId)
            ->whereBetween('created_at', [$startOfLastMonth, $endOfLastMonth])
            ->count();

        $currentEarnings = Booking::where('doctor_id', $doctorId)
            ->whereBetween('created_at', [$startOfCurrentMonth, $endOfCurrentMonth])
            ->sum('total_amount');

        $lastMonthEarnings = Booking::where('doctor_id', $doctorId)
            ->whereBetween('created_at', [$startOfLastMonth, $endOfLastMonth])
            ->sum('total_amount');

        $currentNewPatients = UserProfile::where('link_id', $doctorId)
            ->whereBetween('created_at', [$startOfCurrentMonth, $endOfCurrentMonth])
            ->distinct('id')
            ->count();

        $lastMonthNewPatients = UserProfile::where('link_id', $doctorId)
            ->whereBetween('created_at', [$startOfLastMonth, $endOfLastMonth])
            ->distinct('id')
            ->count();
        ...
        $appointmentChange = calculatePercentageChange($currentAppointments, $lastMonthAppointments);
        $earningsChange = calculatePercentageChange($currentEarnings, $lastMonthEarnings);
        $newPatientsChange = calculatePercentageChange($currentNewPatients, $lastMonthNewPatients);

        // Determine if it's increased or decreased
        $appointmentStatus = $appointmentChange >= 0 ? 'High' : 'Low';
        $earningsStatus = $earningsChange >= 0 ? 'High' : 'Low';
        $newPatientsStatus = $newPatientsChange >= 0 ? 'High' : 'Low';
        $recentBookings = Booking::where('doctor_id', $doctorId)
            ->latest('created_at')
            ->take(5)
            ->get();
        return view('doctor.dashboard', ['recentBookings' => $recentBookings,
            'currentAppointments' => $currentAppointments,
            'appointmentChange' => round($appointmentChange, 2) . '%',
            'appointmentStatus' => $appointmentStatus,

            'currentNewPatients' => $currentNewPatients,
            'newPatientsChange' => round($newPatientsChange, 2) . '%',
            'newPatientsStatus' => $newPatientsStatus,

            'currentEarnings' => $currentEarnings,
            'earningsChange' => round($earningsChange, 2) . '%',
    ...
    public function export_online_astrologer(Request $request)
    {
        if (isset($_GET['onlineExport'])) {
            $data[] = array(
                ...
                "normal_chat_price",
                "priority_chat_price",
                "scheduled_chat_price",
                "normal_call_price",
                "priority_call_price",
                "scheduled_call_price",
                "normal_video_price",
                "priority_video_price",
                "scheduled_video_price",
                "normal_chat_price_usd",
                "priority_chat_price_usd",
                "scheduled_chat_price_usd",
                "normal_call_price_usd",
                "priority_call_price_usd",
                "scheduled_call_price_usd",
                "normal_video_price_usd",
                "priority_video_price_usd",
                "scheduled_video_price_usd",
            );
            ...
            foreach ($counts as $count) {
                ...
                $aoe = '';
                if ($count->getastroservice()) {
                    $asaoe = array();
                    foreach ($count->getastroservice() as $key) {
                ...
                $lngg = '';
                if ($count->getlangauge()) {
                    $langauge = array();
                
                    foreach ($count->getlangauge() as $key) {
                ...
                $data[] = array(
                    ...
                    $aoe,
                    $lngg,
                    $count->pricedetails->normal_chat_price ?? 0,
                    $count->pricedetails->priority_chat_price ?? 0,
                    $count->pricedetails->scheduled_chat_price ?? 0,
                    $count->pricedetails->normal_call_price ?? 0,
                    $count->pricedetails->priority_call_price ?? 0,
                    $count->pricedetails->scheduled_call_price ?? 0,
                    $count->pricedetails->normal_video_price ?? 0,
                    $count->pricedetails->priority_video_price ?? 0,
                    $count->pricedetails->scheduled_video_price ?? 0,
                    $count->pricedetails->normal_chat_price_usd ?? 0,
                    $count->pricedetails->priority_chat_price_usd ?? 0,
                    $count->pricedetails->scheduled_chat_price_usd ?? 0,
                    $count->pricedetails->normal_call_price_usd ?? 0,
                    $count->pricedetails->priority_call_price_usd ?? 0,
                    $count->pricedetails->scheduled_call_price_usd ?? 0,
                    $count->pricedetails->normal_video_price_usd ?? 0,
                    $count->pricedetails->priority_video_price_usd ?? 0, 
                    $count->pricedetails->scheduled_video_price_usd ?? 0, 
                );
