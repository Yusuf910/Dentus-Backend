                ...
                if ($oop->count() > 0) {
                    foreach ($oop as $us) {
                        ...
                        $dataexport[] = [
                            ...
                            $a_name5,
                            // $variant,
                            number_format((double)$us->price, 2, '.', ''),
                            $us->qty,
                            number_format((double)($us->total_price - $us->total_tax), 2, '.', ''),
                            $taxrate1.','.$taxrate2,
                            $taxtype1.','.$taxtype2,
                            $us->total_tax,
                            $us->total_price
        ...
        if($st == 1) {
            ...
            if (!empty($user->astrologer_id)) {
                ...
                if ($commision_percent > 0) {
                    ...
                    $astrologer_commission = number_format((double)$astrologer_commission, 2, '.', '');
                    $tds_amount = number_format((double)$tds_amount, 2, '.', '');
                    // $gst_amount = number_format((double)$gst_amount, 2, '.', '');
                    $admin_commision = number_format((double)($user->price - $astrologer_commission), 2, '.', '');
                }
