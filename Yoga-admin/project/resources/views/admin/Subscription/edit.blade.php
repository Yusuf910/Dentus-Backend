                           ...
                           <div class="col-md-6">
                              <div class="mb-3">
                                 ...
                                 <select class="form-select" id="package-name" name="name" required>
                                    ...
                                    <option value="Free Trial" {{ $a->name == 'Free Trial' ? 'selected' : '' }}>Free Trial</option>
                                    <option value="Monthly" {{ $a->name == 'Monthly' ? 'selected' : '' }}>Monthly</option>
                                    <option value="Yearly" {{ $a->name == 'Yearly' ? 'selected' : '' }}>Yearly</option>
                                 </select>
                              </div>
                           </div>
                           <div class="col-md-6">
                              <div class="mb-3">
                                 ...
                                 <input value="{{ $a->price }}" name="price" min="0" class="form-control" type="number" id="package-price" placeholder="eg. 0" required>
                              </div>
                           </div>
                           <div class="col-md-6">
                              <div class="mb-3">
                                 <label for="discount-price" class="form-label">Discount Price</label>
                                 <input value="{{ $a->discount_price }}" name="discount_price" min="0" class="form-control" type="number" id="discount-price" placeholder="eg. 0">
                              </div>
                           </div>
                           <div class="col-md-6">
                              <div class="mb-3">
                                 <label for="validity-days" class="form-label">Validity Days</label>
                                 <input value="{{ $a->validity_days }}" name="validity_days" min="1" class="form-control" type="number" id="validity-days" placeholder="eg. 7" required>
                              </div>
                           </div>
                           <div class="col-md-6">
                              <div class="mb-3">
                                 ...
                                 <option value="free" {{ $a->type == 'free' ? 'selected' : '' }}>Free</option>
                                 <option value="paid" {{ $a->type == 'paid' ? 'selected' : '' }}>Paid</option>
                                 </select>
                              </div>
                           </div>
                           <div class="col-md-6">
                              <div class="mb-3">
                                 ...
                                 <select name="taxId" class="form-select">
                                    ...
                                    @foreach ($taxes as $p)
                                    <option value="{{ $p->_id }}" {{ $a->taxId == $p->_id ? 'selected' : '' }}>
                           ...
                           <div class="col-12">
                              <div class="mb-3">
                                 ...
                                 <div id="features-container">
                                    @foreach($a->what_you_got as $index => $feature)
                                    <div class="feature-item row mb-2">
                                       <div class="col-md-5">
                                          <select name="what_you_got[{{ $index }}][posesId]" class="form-select" required>
                                             <option value="">Select Pose</option>
                                             @foreach($posesList as $pose)
                                             <option value="{{ $pose->_id }}" {{ $feature['posesId'] == $pose->_id ? 'selected' : '' }}>
                                             {{ htmlspecialchars($pose->name, ENT_QUOTES, 'UTF-8') }}
                                             </option>
                                             @endforeach
                                          </select>
                                       </div>
                                       <div class="col-md-5">
                                          <select name="what_you_got[{{ $index }}][is_free]" class="form-select" required>
                                          <option value="true" {{ $feature['is_free'] == 1 ? 'selected' : '' }}>True</option>
                                          <option value="false" {{ $feature['is_free'] == 0 ? 'selected' : '' }}>False</option>
                                          </select>
                                       </div>
                                       <div class="col-md-2">
                                          <input type="hidden" name="what_you_got[{{ $index }}][_id]" value="{{ $feature['_id'] ?? '' }}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    addButton.addEventListener('click', function() {
        ...
        newFeature.innerHTML = `
            ...
            <div class="col-md-2">
                <input type="hidden" name="what_you_got[${featureCount}][_id]" value="${(new Date()).getTime()}">
                <button type="button" class="btn btn-danger remove-feature">Remove</button>
            </div>
        `;
    ...
    function reindexFeatureItems() {
        ...
        featureItems.forEach((item, index) => {
            ...
            const idInput = item.querySelector('input[name^="what_you_got"][name$="[_id]"]');
            
            poseSelect.name = `what_you_got[${index}][posesId]`;
            isFreeSelect.name = `what_you_got[${index}][is_free]`;
            if(idInput) {
                idInput.name = `what_you_got[${index}][_id]`;
            }
        });
    }
});
