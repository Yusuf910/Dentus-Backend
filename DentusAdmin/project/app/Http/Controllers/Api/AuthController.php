...
class AuthController extends Controller
{
    ...
    public function register($data, $user_type, $login_type, $created_by = null)
    {
        ...
        $invitation = DB::table('Invitation')->where('email',$data['email'])->where('mobile',$data['mobile'])->where('status',1)->first();
        // dd($invitation);
        if ($invitation->status == 0) {
            return response()->json(['message' => 'Your status is not approve'], 400);
        }
        $data['email'] = trim($data['email']);
        $data['password'] = Hash::make($data['password']);
        $data['status'] = 3;
        $data['parent_id'] = $invitation->doctor_id;
        $user = User::create($data);
        $token = $user->createToken('MyApp')->accessToken;
        DB::table('Invitation')->where('id', $invitation->id)->update(['user_id' => $user->id]);
    ...
    public function login($data, $user_type)
    {
        ...
        if ($user) {
            ...
            $response = ['status' => false, 'msg' => 'Email or Password is wrong.'];
    ...
    public function sendOtpForForgotPassword(Request $request)
    {
        ...
        if ($user) {
            ...
            if ($user->email) {
                ...
                // $view  = view('emails.otpapp',compact('data'))->render();
                // $subject = "Pegasus Verification Code";
                // $mail = $this->check_curl_for_sendinblue($user->email,$user->name,$subject,$view,'otp');
            }

            $response = ['status' => true, /*'otp' => $otp,*/ 'msg' => 'Forgot password verification otp send to your mobile. Please verify to continue.', 'user_id' => $user->id];
        } else {
