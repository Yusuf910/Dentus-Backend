                  ...
                  {!! Form::model($a, ['method' => 'PATCH','enctype' => 'multipart/form-data','route' => [$editurl, $a->id]]) !!}
                     ...
                     <div class="row">
                        ...
                        <div class="form-group row">
                           ...
                           <div class="form-group row">
                              <label for="example-text-input" class="col-sm-2 col-form-label">Subcategory Name <span class="text-danger">*</span></label>
                              <div class="col-sm-10">
                              <select class="form-control" name="sub_category_id" required>
                                    <option value="">Select Name</option>
                                    <?php $check = DB::table('suggestion_sub_categories')->where('status',1)->get();
                                    foreach ($check as $n){
                                       ?>
                                   <option value="{{ $n->id }}"
                                    <?php
                                    if($n->id==$a->sub_category_id){
                                        echo "selected";
                                    }
                                    else{
                                        echo "";
                           ...
                           <div class="form-group row">
                              <label for="example-text-input" class="col-sm-2 col-form-label">Message <span class="text-danger">*</span></label>
                              <div class="col-sm-10">
                                 <input required value="{{ $a->message }}" class="form-control" type="text" id="example-text-input" placeholder="Enter name" name="message">
                              </div>
                           </div>
